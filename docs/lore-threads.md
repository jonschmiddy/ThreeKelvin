...
- **Commission three hundred and eleven.** The Halcyon rider — the perpetuity clause, the hand that has signed every commission since the first — is commission 311. Nothing else references it yet. It is the best hook in the archive and it is currently baited with nothing.
...
Seven threads. Each lists its anchors (existing entries), its new entries drafted in full, and what it is for. Entry format matches `Database._doc`: id, title, author-with-a-job, its own date in its own epoch, house, depth floor.
...
**Anchor:** `halcyon_rider` (countersigned by a hand that has signed every commission since the first).
...
A thread (§3) is a question that stays open because the thing asking it is an institution, and institutions do not end. An arc is a *person*, followed through three or four documents, whose story completes — with a death, a retirement, a capitulation, a refusal, a walking-away. People are temporary; a setting whose entire idea is that people end owes its people endings. The houses get threads. The humans get arcs. That asymmetry is the one idea restated as narrative structure, which is why it costs nothing to add.
...
**Place-name collisions, for free.** Documents that name places should draw those names from `MapGen.star_name`'s vocabulary. The archive persists across runs; galaxies do not; so one day a player docks at a system whose name they read in a Dredge filing two ships ago. Nothing needs to detect this. The collision is the content, `found_at` already stamps star names into read entries, and the generator's syllables are the only coordination required. (`dredge_filing`'s "Kestrel Reach" should be retro-checked against the generator so it is a name the sky can actually produce.)
...
| `halcyon_attendance` | Japanese | `expense_claim` | Hungarian |
| `halcyon_exemplar` | French | `repair_declined` | Greek |
