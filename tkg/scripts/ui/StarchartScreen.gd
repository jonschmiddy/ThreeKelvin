...
class MapChart extends Control:
	...
	## fiction pays for it: the stoker is the hottest thing flying, and heat is
	...
	func _draw_stoker() -> void:
		if not show_icons or not Run.stoker_alive() \
				or Run.stoker_at >= Run.map.size():
			return
		var c := _screen_pos(Run.map[Run.stoker_at])
		_diamond(c, 7.0, UITheme.VOID)
		_diamond(c, 6.0, UITheme.EMBER)
		var at_text := c + Vector2(-46, -14)
		draw_string(UITheme.pixel_font(), at_text + Vector2(1, 1), "THE STOKER",
			HORIZONTAL_ALIGNMENT_CENTER, 92, 8, Color(0, 0, 0, 0.85))
		draw_string(UITheme.pixel_font(), at_text, "THE STOKER",
