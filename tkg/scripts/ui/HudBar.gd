...
func refresh() -> void:
	...
	# same reason CARDS is — looking at past runs cannot affect this one. Locked
	# while choosing a chassis, like everything else on the bar: the one screen
	# where the only thing to do is LAUNCH.
	_row.add_child(_tab("HISTORY", Router.current is HistoryScreen, choose_lock,
