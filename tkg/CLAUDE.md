...
Pixel-art turn-based roguelite. Godot 4.7.1, GDScript. Solo developer.
...
| `-- content` | adding cards or modules. A MEASUREMENT, not a gate: prints unique cards per manufacturer against the target — Korvan 40, unbranded 20, malfunctions 15 — and passes either way. Counts unique NAMES, so two modules granting the same verb are one card |
| `-- parts` | touching modules, `ModuleIcon` or the hold's cell size. Every module in the game at its own footprint, grouped by manufacturer. The sibling of `-- cards`, and dev-only for the same reason |
...
| **Malfunctions are three keywords, not sixteen sentences** | **Corrode n** is damage at end of turn while it is in your hand, **Smoulder n** is heat the same way, and **Fused** means it is not discarded at end of turn. Sixteen cards written as sixteen sentences came out as three behaviours with different numbers; written as combinations they are sixteen a player can read at a glance, which is how Brace, Salvo and Charge already work. Eight of them are severities of the same two verbs on purpose — Corrode 1/2/3 is a ladder you learn, not three rules |
| **Fused is the only malfunction keyword that COMPOUNDS** | Everything else charges you once and goes with the hand. A fused card charges you again every turn and holds a hand slot the whole time, so it is the one worth spending a Discard on — and a Decommission if you would rather it did not come back at all. It is discardable on purpose: a malfunction that ONLY Decommission could remove would make the cheap verb useless against exactly the cards it exists for |
| **`Run.dross` is WHICH malfunctions you carry, not how many** | It was a count, and a count can only ever produce one card sixteen times. The ids are stored so the deck rebuilds identically on every load and every deck-build — rolling which malfunction at build time would hand you a different set of junk every time the screen refreshed. A save written when it was a count restores as that many Dross, which is exactly what it meant then |
...
| **Two verbs for losing a card, and they are two different sizes of decision** | REVERSES "discard verbs never ask the player to choose", which was a limit dressed as a design. **Discard** is the ordinary one — you pick, it goes to the pile, it is back in the deck the moment that pile reshuffles; `discard_hand` is the same verb at the scale of a hand and pairs with `draw`, a real cost because it throws away what you wanted too. **Decommission** is the expensive one: off the books for the rest of the fight. Against junk that is the whole difference, because a discarded malfunction comes back and a decommissioned one does not. `self_decommission` is a card that takes ITSELF off the books, which is how a card earns the right to be much stronger than its cost. The word is the setting's own register: a universe of filings, riders and invoices strikes a part off a register rather than throwing it away |
...
| **A card belongs to the manufacturer of the part that granted it** | `resolved_cards()` stamps the module's manufacturer onto every card it produces, so a SHARED card granted by a Korvan plate is a Korvan card — it counts toward Korvan's set, takes Korvan's colour, and Verity's "Verity cards cost 1 less" already depends on exactly this. Which is why the shared vocabulary is not renamed per manufacturer: one Bolt On six manufacturers grant is one card to learn, six differently-named Bolt Ons is six |
| **THE CARD RARITY LAW: cells decide what grades a module may grant** | 1-2 cells is one card at the part's own rarity and one BELOW it; 3-4 cells is two at the part's rarity. So a legendary 4x1 does grant two legendary cards, and that is fine because it costs a whole row of a light hull's twelve — the power is bought with hold space rather than handed over, and a legendary 1x1 could not be allowed the same thing. "Below it" and not "one step below", which is what lets the shared commons sit under anything without a special case. A function of the part like `grant_count`, so "a small part never grants two top-grade cards" is true by construction; `-- holdtest` holds the line for the cards that DECLARE a rarity, which is the only path that can get it wrong |
...
| **The profit is in the distance between two places, not in one transaction** | Round-tripping a part where you stand is a guaranteed loss, and that is correct: a scrapyard is not a market. A manufacturer's own yard is thick with its own parts and pays a glut price; a rival's yard needs what it cannot press. Buy Korvan in Korvan space, sell it to Solari. The route IS the trade |
...
| **Materials are prerequisites, not a second currency** — and ALLOY failed that test and is gone | A recipe costing forty credits is a purchase; a recipe costing one precursor fragment is a reason to have flown somewhere. That is how crafting gets a cost credits cannot pay without breaking the one-currency ruling. Alloy broke it: it came off every part you broke down, so it accrued from the hold you were emptying anyway rather than from anywhere you WENT — a faucet that large is a second currency whatever the ruling calls it. Exotic and relic survive because their sources are places (megafauna, pulsars, deep wrecks). `HULL PATCH` and `FUEL SYNTHESIS` went with it, being the two recipes alloy paid for. `COOLANT BRAID` outlived alloy and died of a different ruling — heat capacity comes only from thermal modules — which leaves `RELIC ANALYSIS` as the whole of the fabricator until the next recipe is written |
| **Heat capacity comes off a thermal module and nowhere else** | Capacity is the one number a whole slot of parts exists to move, so every other source of it was competing with the parts you bolt on. Three went: the Yard's `+2 HEAT CAP` service (credits at a counter, which made the thermal ladder optional), the fabricator's `COOLANT BRAID`, and the `heat_sink` hull perk. The perk left four grade ladders short, so each got a substitute AT THE SAME INDEX — the index is the grade, and a two-entry ladder means S confers nothing over A. `Run.heat_cap_bonus` survives with no setter because it is a SAVED field: retiring it is a save-shape change and a version bump to delete a value that reads 0 |
