...
| `-- fittest` | touching the hold, the hardpoints, `ModuleIcon` or either drop handler. **Needs a window, so it is NOT in the gate** — it drives real drags. Four crossings between the hold and the hull, both directions and a refusal. The return journey was unreachable for weeks and looked written: `_on_hold_drop` had always handled a part arriving off the ship and nothing could start that drag |
