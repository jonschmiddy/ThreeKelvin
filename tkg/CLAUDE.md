...
| `docs/building-a-house.md` | **Before writing a manufacturer's parts.** The sequence, the four gates to run, the six audit questions, and the traps that cost time on the first two houses |
...
| **A card belongs to the house of the part that granted it** | `resolved_cards()` stamps the module's manufacturer onto every card it produces, so a SHARED card granted by a Korvan plate is a Korvan card — it counts toward Korvan's set, takes Korvan's colour, and Verity's "Verity cards cost 1 less" already depends on exactly this. Which is why the shared vocabulary is not renamed per house: one Bolt On six houses grant is one card to learn, six differently-named Bolt Ons is six |
