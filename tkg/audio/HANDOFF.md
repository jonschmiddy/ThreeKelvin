...
142 and 71 BPM are deliberate — exact 2:1, so the two pieces are bar-aligned with each other.
...
- Extract the score DSL — the `(note, beat, duration)` tuple lists and cycle loops are the same pattern in both files and want to be a shared module.
